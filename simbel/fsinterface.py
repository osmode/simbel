...
class FSInterface:
	...
	def download_all_files(self,bci,walk_dir=None):
		try:
			...
			while i < num_records:
				...
				#bashcommand="mv "+ipfs_hash+" "+os.path.join(os.getcwd(),'simbel/share')+"/"+filename
	...
	def upload_all_files(self,bci,walk_dir=None):
		all_files=[]
		if not walk_dir:
			walk_dir=os.path.join(os.getcwd(), 'simbel/share') 		
	...
	def my_enode(self):
		fname=os.getcwd()+'/simbel/nodeInfo.ds'
		with open(fname) as f:
			content=f.readlines()

		if len(content) !=2:
			print("Invalid enode read in /simbel/nodeInfo.ds")
			return None
	...
	def add_static_node(self,enode):
		fname_path=os.getcwd()+'/simbel/data/static-nodes.json'	   
		...
		line_number = 0
		temp_file_path=os.getcwd()+'/simbel/data/static-nodes2.json'
